...
# Практика: UX-валидация в реальной форме регистрации
...
if (!FormUtils.validatePassword(password)) {
  FormUtils.showFieldError("password", "Пароль не соответствует требованиям");
}
```

Вся логика UX-валидации реализована в файле `register.js` и синхронизирована с интерфейсом.
...
class EnhancedValidator {
  ...
  initDefaultValidators() {
    ...
    this.addValidator("name", {
      ...
      message:
        "Имя должно содержать 2-50 символов (только русские буквы, пробелы и дефисы)",
  ...
  initAuthValidators() {
    ...
    this.addValidator("passwordStrength", {
      ...
      message: (result) => {
        ...
        return failed.length > 0 ? `Добавьте: ${failed.join(", ")}` : "";
      },
...
class ProgressiveValidator extends AuthValidator {
  ...
  async validateCurrentStep(form) {
    ...
    const isStepValid = Array.from(results.values()).every(
  ...
  createHintElement(content, type, context) {
    ...
    hint.innerHTML = `
            <div class="smart-hint__content">
                ...
                ${
                  context.suggestion
                    ? `<div class="smart-hint__suggestion">${context.suggestion}</div>`
                    : ""
                }
            </div>
  ...
  positionHint(hintElement, targetField, position) {
    ...
    switch (position) {
      case "top":
        ...
        left =
          fieldRect.left + scrollLeft + (fieldRect.width - hintRect.width) / 2;
      ...
      case "left":
        top =
          fieldRect.top + scrollTop + (fieldRect.height - hintRect.height) / 2;
