...
class AuthApplication {
    ...
    setupEventHandlers() {
        ...
        this.eventBus.on('auth:token:refresh:success', this.handleTokenRefreshSuccess.bind(this));
        this.eventBus.on('auth:token:refresh:error', this.handleTokenRefreshError.bind(this));
...
class AuthAPI {
        ...
        try {
            ...
            for (const interceptor of this.interceptors.response) {
                if (interceptor.onSuccess) {
                    finalResponse = await interceptor.onSuccess(finalResponse) || finalResponse;
                }
            }
    ...
    async confirmReset(token, newPassword) {
        const response = await this.post('/auth/confirm-reset', { token, newPassword });
        return response;
    }
    ...
    setCached(key, data, ttl = 300000) { // 5 минут по умолчанию
        this.cache.set(key, {
            data: data,
